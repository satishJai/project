...
KSColorMatching::KSColorMatching()
{
	...
	k = RosParameterManager::createParam<int> (5,ParamType::INT,0,50,"/matching/kcolorMatching/k");
}

void KSColorMatching::run(Descriptors& descriptors, pcl::PointCloud<PointType>::ConstPtr keypoints, const std::vector<Model>* models,
	...
	for (unsigned long j = 0; j < models->size (); j++)
	{
		...
		for (unsigned long i = 0; i < descriptors.getSize (); ++i)
		{
			...
			}else //okay, lets do a k nearest neightbor search, and check if we get a color match there
			{
				...
				bool match = false;
				PointType startKeypoint = keypoints->at(i);
				...
				if ( tree.nearestKSearch (startKeypoint, k->var, pointIdxNKNSearch, pointNKNSquaredDistance) > 0 ) //Can we find neighbours?
				{
					while((count < k->var) && !match)//
					{
						...
						{
							if(isSimilarColor(&keypoints->at(pointIdxNKNSearch.at(count)), &model.data.keypoints->at(corres.index_query),
		...
		notFilteredCorresCountVec->push_back(notFilteredCorresCount);
		...
		model_scene_corrs_vec->push_back(model_scene_corrs);
	}
}
bool KSColorMatching::isSimilarColor(const PointType *scene, const PointType *model, double nBlackThreshold, double nGrayThreshold, double nAngleThresh, double nValueThresh){
...
KSColorMatching::hsv KSColorMatching::rgb2hsv (rgb in)
{
	hsv out;
...
KSColorMatching::rgb KSColorMatching::hsv2rgb (hsv in)
{
	double hh, p, q, t, ff;
	long i;
	rgb out;
